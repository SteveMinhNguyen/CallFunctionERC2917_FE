...
import ERC2917ImplABI from "./abi/ERCImplABI";
...
export var Web3Query = {
    ...
    queryTokenList() {
        ...
        return new Promise(f => {
            setTimeout(async () => {
                try {
                    results = await queryContract.methods
                        .queryTokenList()
                    ...
                    for (let i = 0; i < results.length; i++) {
                        ...
                        let balance = Web3Storage.isETH(item["tokenAddress"])
                            ? await Web3Provider.getBalance()
                            : convertBigNumberToNormal(
                                  item["balance"],
                                  decimals
                              );
                        let allowance = Web3Storage.isETH(item["tokenAddress"])
                            ? Web3Storage.MAXNUM
                            : convertBigNumberToNormal(
                                  item["allowance"],
                                  decimals
                              );
                        let symbol = Web3Storage.isETH(item["tokenAddress"])
                            ? Web3Provider.getZeroSymbol()
                            : item["symbol"];
                        let totalSupply = Web3Storage.isETH(
                            item["tokenAddress"]
                        )
                            ? 0
                            : convertBigNumberToNormal(
                        ...
                        let allowance_gov = convertBigNumberToNormal(
                            item["allowanceGov"],
                            decimals
                        );
                        tokenList.push(
    ...
    async getBalanceOf() {
        let balance = 0;
        try {
            balance = await queryContract.methods
                .balanceOf("0x9C53c277c6F4f3C3C64Fa3C484CB1875e55B27C9")
                .call({ from: String(Web3Provider.currentAccount) });
            console.log("balance from query");
            console.log(balance);
        } catch (err) {
            alert(err);
        }

        return convertBigNumberToNormal(balance, 18);
    },
    async getName() {
        let name;
        try {
    ...
    async queryLiquidityList() {
        ...
        let liquidityList = [];
        let results = await queryContract.methods
            .queryLiquidityList()
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            let canRemove =
                block >=
                parseInt(l["lastBlock"]) + Web3Storage.params.removeDuration;
        ...
        results = await queryContract.methods
            .queryPairListInfo(address_list)
            .call({ from: Web3Provider.currentAccount });
        ...
        if (address_list && address_list.length > 0) {
            let res = await Web3Query.QPost(
                Web3Provider.getOpenApiUrl() + "/market/pair/aprs",
                { pairIDs: address_list }
            );
            if (res && res.data && res.code === 2000) {
                for (let i = 0; i < address_list.length; i++) {
                    if (res.data[i]) {
                        liquidityList[i].setApr(
                            res.data[i].aprTransaction,
                            res.data[i].aprMining
                        );
                    }
                }
            }
    ...
    async pageSliderProposalList() {
        ...
        return new Promise((resolve, reject) => {
            const pageSlider = async (start, end) => {
                ...
                try {
                    let results = await queryContract.methods
                        .iterateReverseProposalList(start, Math.max(end, 0))
                        .call({ from: Web3Provider.currentAccount });
                    for (let i = 0; i < results.length; i++) {
                        let item = results[i];
                        let proposal = new Proposal(
                            ...
                            item.createTime,
                            item.end,
                            convertBigNumberToNormal(item.YES),
                            convertBigNumberToNormal(item.NO),
                            convertBigNumberToNormal(item.totalReward),
                            item.voted,
                            item.voteIndex,
                            item.minted,
                            convertBigNumberToNormal(item.weight),
                            item.audited
                        );
                        proposal.remainBlock = Math.max(
                            0,
                            parseInt(item.endBlock) - block
                        );
                        ...
                        if (parseInt(proposal.type) === 1) {
                            ...
                            proposal.setValue(
                                convertBigNumberToNormal(item.value, decimal)
                            );
                            proposal.key = key;
                            proposal.currentValue = convertBigNumberToNormal(
                                item.currentValue,
                                decimal
                            );
                        }
    ...
    async iterateReverseProposalList(start, end) {
        ...
        let proposalList = [];
        let results = await queryContract.methods
            .iterateReverseProposalList(start, end)
            .call({ from: Web3Provider.currentAccount });
        for (let i = 0; i < results.length; i++) {
            let item = results[i];
            let proposal = new Proposal(
            ...
            if (parseInt(proposalList[i].type) === 1) {
                let key = Web3Provider.fromByte32(item.key);
                let decimal = Web3Storage.getConfigDecimal(key);
                proposalList[i].setValue(
                    convertBigNumberToNormal(item.value, decimal)
                );
                proposalList[i].key = key;
                proposalList[i].currentValue = convertBigNumberToNormal(
                    item.currentValue,
                    decimal
                );
            }
    ...
    async queryConfig() {
        ...
        Web3Storage.setParams(
            feePercent,
            parseFloat(convertBigNumberToNormal(result[1])),
            parseInt(result[2]),
            parseInt(result[3]),
            parseFloat(convertBigNumberToNormal(result[4])),
            rewardPercent
        );
    },

    async queryConfigInfo(key) {
        let result = await queryContract.methods
            .queryConfigInfo(Web3Provider.web3.utils.fromAscii(key))
            .call();
        ...
        return Web3Storage.setConfigInfo(
            key,
            minValue,
            maxValue,
            currentValue,
    ...
    async queryNewToken(address) {
        try {
            ...
            return {
                ...
                balance: convertBigNumberToNormal(
                    result["balance"],
                    parseInt(result["decimal"])
                ),
                totalSupply: convertBigNumberToNormal(
                    result["totalSupply"],
                    parseInt(result["decimal"])
                ),
                allowance: convertBigNumberToNormal(
                    result["allowance"],
                    parseInt(result["decimal"])
                ),
    ...
    async queryStakeInfo() {
        ...
        let stakeAmount = parseFloat(
            convertBigNumberToNormal(result.stake_amount)
        );
        let stakeInfo = {
            totalStake: convertBigNumberToNormal(result.total_stake),
            stakeAmount: stakeAmount,
            redeemAmount:
                parseInt(result.stake_block) +
                    Web3Storage.params.unstakeDuration >
                block
                    ? 0
                    : stakeAmount,
            remainBlock:
    ...
    async queryCondition(tokenList) {
        return await queryContract.methods
            .queryCondition(tokenList)
            .call({ from: Web3Provider.currentAccount });
    },

    async queryReserveList(token0_list, token1_list) {
        let result = await queryContract.methods
            .queryPairReserve(token0_list, token1_list)
            .call({ from: Web3Provider.currentAccount });
        for (let i = 0; i < token0_list.length; i++) {
            let reserve0 = convertBigNumberToNormal(
                result.reserve0_list[i],
                Web3Storage.getDecimal(token0_list[i])
            );
            let reserve1 = convertBigNumberToNormal(
                result.reserve1_list[i],
                Web3Storage.getDecimal(token1_list[i])
            );
            let pair = new Pair(
                token0_list[i],
                token1_list[i],
