    ...
    actions: {
        ...
        getCoinInfo({ commit }) {
            return SwapInstance.$query.queryTokenList().then(info => {
                const tokenInfo = info.find(d =>
                    Web3Provider.isDGASAddress(d.address)
                );
        ...
        queryConfigInfo({ state }, val) {
            ...
            return SwapInstance.$query.queryConfigInfo(val).then(k => {
                if (
                    Number(k.maxValue) >
                    Number(k.span) + Number(k.currentValue)
                ) {
                    k.maxValue = Number(k.span) + Number(k.currentValue);
                }
                if (
                    Number(k.minValue) <
                    Number(k.currentValue) - Number(k.span)
                ) {
        ...
        getUnit({ state }, { key }) {
            return (
                SwapInstance.$constant.BallotList.find(k => k.key === key)
                    ?.unit || null
            );
        },
        ...
        implement({ state }, { address, cb }) {
            return SwapInstance.$governance.auditConfig(
                address,
                (proposal, tx) => {
                    cb(proposal, tx);
                }
